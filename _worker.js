				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							const 地区探测接口 = String(env.REGION_PROBE_API || '').trim();
							log(`[地区前缀] mixed订阅开始，原始优选数量=${完整优选IP.length}，接口已配置=${Boolean(地区探测接口)}，UA=${ua}`);
							if (地区探测接口) 完整优选IP = await 批量补全优选地址地区前缀(完整优选IP, env);
							log(`[地区前缀] mixed订阅处理完成，输出优选数量=${完整优选IP.length}`);
...
const 优选地区缓存 = new Map();
const 地区简称集合 = new Set(['AD', 'AE', 'AF', 'AG', 'AI', 'AL', 'AM', 'AO', 'AQ', 'AR', 'AS', 'AT', 'AU', 'AW', 'AX', 'AZ', 'BA', 'BB', 'BD', 'BE', 'BF', 'BG', 'BH', 'BI', 'BJ', 'BL', 'BM', 'BN', 'BO', 'BQ', 'BR', 'BS', 'BT', 'BV', 'BW', 'BY', 'BZ', 'CA', 'CC', 'CD', 'CG', 'CH', 'CI', 'CK', 'CL', 'CM', 'CN', 'CO', 'CR', 'CU', 'CV', 'CW', 'CX', 'CY', 'CZ', 'DE', 'DJ', 'DK', 'DM', 'DO', 'DZ', 'EC', 'EE', 'EG', 'EH', 'ER', 'ES', 'ET', 'FI', 'FJ', 'FK', 'FM', 'FO', 'FR', 'GA', 'GB', 'GD', 'GE', 'GF', 'GG', 'GH', 'GI', 'GL', 'GM', 'GN', 'GP', 'GQ', 'GR', 'GS', 'GT', 'GU', 'GW', 'GY', 'HK', 'HM', 'HN', 'HR', 'HT', 'HU', 'ID', 'IE', 'IL', 'IM', 'IN', 'IO', 'IQ', 'IR', 'IS', 'IT', 'JE', 'JM', 'JO', 'JP', 'KE', 'KG', 'KH', 'KI', 'KM', 'KN', 'KP', 'KR', 'KW', 'KY', 'KZ', 'LA', 'LB', 'LC', 'LI', 'LK', 'LR', 'LS', 'LT', 'LU', 'LV', 'LY', 'MA', 'MC', 'MD', 'ME', 'MF', 'MG', 'MH', 'MK', 'ML', 'MM', 'MN', 'MO', 'MP', 'MQ', 'MR', 'MS', 'MT', 'MU', 'MV', 'MW', 'MX', 'MY', 'MZ', 'NA', 'NC', 'NE', 'NF', 'NG', 'NI', 'NL', 'NO', 'NP', 'NR', 'NU', 'NZ', 'OM', 'PA', 'PE', 'PF', 'PG', 'PH', 'PK', 'PL', 'PM', 'PN', 'PR', 'PS', 'PT', 'PW', 'PY', 'QA', 'RE', 'RO', 'RS', 'RU', 'RW', 'SA', 'SB', 'SC', 'SD', 'SE', 'SG', 'SH', 'SI', 'SJ', 'SK', 'SL', 'SM', 'SN', 'SO', 'SR', 'SS', 'ST', 'SV', 'SX', 'SY', 'SZ', 'TC', 'TD', 'TF', 'TG', 'TH', 'TJ', 'TK', 'TL', 'TM', 'TN', 'TO', 'TR', 'TT', 'TV', 'TW', 'TZ', 'UA', 'UG', 'UM', 'US', 'UY', 'UZ', 'VA', 'VC', 'VE', 'VG', 'VI', 'VN', 'VU', 'WF', 'WS', 'YE', 'YT', 'ZA', 'ZM', 'ZW']);

function 备注已带地区简称(备注 = '') {
	const tokens = String(备注 || '').toUpperCase().split(/[^A-Z]+/).filter(token => token.length === 2);
	return tokens.some(token => 地区简称集合.has(token));
}

function 备注已带离线前缀(备注 = '') {
	return String(备注 || '').toUpperCase().split(/[^A-Z]+/).includes('OFF');
}

function 备注需要补地区(备注 = '', host = '') {
	const 文本 = String(备注 || '').trim();
	if (!文本) return true;
	if (备注已带地区简称(文本)) return false;
	if (备注已带离线前缀(文本)) return false;
	return true;
}
...
function 标准化地区简称(值 = '') {
	const 文本 = String(值 || '').trim().toUpperCase();
	return 地区简称集合.has(文本) ? 文本 : null;
}

function 提取地区结果映射(响应数据, 待查询条目列表 = []) {
	const 地区映射 = new Map();
	const 写入映射 = (host = '', port = '', region = '') => {
		const 地区码 = 标准化地区简称(region);
		const 纯净host = String(host || '').replace(/^\[|\]$/g, '').trim();
		if (!纯净host || !地区码) return;
		const 端口 = String(port || '').trim();
		if (端口) 地区映射.set(`${纯净host}:${端口}`, 地区码);
		地区映射.set(纯净host, 地区码);
	};
	const 写入对象 = (item = {}) => {
		if (!item || typeof item !== 'object') return;
		const host = item.host || item.ip || item.address || item.addr || item.query || item.target;
		const port = item.port || item.server_port || item.target_port || '';
		const region = item.region || item.countryCode || item.country_code || item.country || item.cc || item.loc;
		if (host || region) 写入映射(host, port, region);
	};

	if (Array.isArray(响应数据)) {
		for (const item of 响应数据) 写入对象(item);
		return 地区映射;
	}
	if (!响应数据 || typeof 响应数据 !== 'object') return 地区映射;

	const 列表候选 = [响应数据.data, 响应数据.results, 响应数据.items, 响应数据.targets];
	for (const 候选 of 列表候选) {
		if (Array.isArray(候选)) {
			for (const item of 候选) 写入对象(item);
			return 地区映射;
		}
	}

	const 对象候选 = [响应数据.data, 响应数据.results, 响应数据.items, 响应数据.regions, 响应数据.map];
	for (const 候选 of 对象候选) {
		if (!候选 || Array.isArray(候选) || typeof 候选 !== 'object') continue;
		for (const [key, value] of Object.entries(候选)) {
			if (value && typeof value === 'object') {
				写入对象({ host: key, ...value });
				continue;
			}
			const [host, port = ''] = key.split(':');
			写入映射(host, port, value);
		}
		if (地区映射.size > 0) return 地区映射;
	}

	for (const 条目 of 待查询条目列表) {
		const 直取结果 = 响应数据?.[条目.key] ?? 响应数据?.[条目.host];
		if (!直取结果) continue;
		if (typeof 直取结果 === 'object') 写入对象({ host: 条目.host, port: 条目.port, ...直取结果 });
		else 写入映射(条目.host, 条目.port, 直取结果);
	}
	return 地区映射;
}

async function 请求外部地区探测接口(待查询条目列表 = [], 接口地址 = '', env = {}) {
	const controller = new AbortController();
	const 超时时间 = Math.max(1000, Number.parseInt(env.REGION_PROBE_TIMEOUT || '8000', 10) || 8000);
	const timeoutId = setTimeout(() => controller.abort(), 超时时间);
	const token = String(env.REGION_PROBE_TOKEN || '').trim();
	try {
		const headers = {
		...
		if (token) {
			...
			headers['X-Region-Probe-Token'] = token;
		}
		const response = await fetch(接口地址, {
			method: 'POST',
			headers,
			body: JSON.stringify({
				targets: 待查询条目列表.map(item => ({
					host: item.host,
					port: item.port ? Number(item.port) || item.port : '',
					remark: item.remark || '',
				}))
			}),
		...
		return 提取地区结果映射(await response.json(), 待查询条目列表);
	} finally {
		clearTimeout(timeoutId);
	}
}

async function 调用外部地区探测(待查询条目列表 = [], env = {}) {
	const 接口列表 = String(env.REGION_PROBE_API || '')
		.split(/[\r\n,|]+/)
		.map(item => item.trim())
		.filter(Boolean);
	const 地区映射 = new Map();
	if (待查询条目列表.length === 0 || 接口列表.length === 0) return 地区映射;

	const 批量大小 = Math.max(1, Number.parseInt(env.REGION_PROBE_BATCH_SIZE || '50', 10) || 50);
	let 未命中条目列表 = 待查询条目列表.filter(item => {
		const 缓存值 = 优选地区缓存.get(item.key) || 优选地区缓存.get(item.host);
		if (!缓存值) return true;
		地区映射.set(item.key, 缓存值);
		return false;
	});
	if (未命中条目列表.length === 0) return 地区映射;

	for (const 接口地址 of 接口列表) {
		if (未命中条目列表.length === 0) break;
		for (let i = 0; i < 未命中条目列表.length; i += 批量大小) {
			const 当前批次 = 未命中条目列表.slice(i, i + 批量大小);
			try {
				const 当前结果 = await 请求外部地区探测接口(当前批次, 接口地址, env);
				for (const item of 当前批次) {
					const 地区码 = 当前结果.get(item.key) || 当前结果.get(item.host);
					if (!地区码) continue;
					地区映射.set(item.key, 地区码);
					优选地区缓存.set(item.key, 地区码);
					优选地区缓存.set(item.host, 地区码);
				}
			} catch (error) {
				log(`[地区前缀] 外部接口失败 ${接口地址}: ${error?.message || error}`);
			}
		}
		未命中条目列表 = 未命中条目列表.filter(item => !地区映射.has(item.key));
	}
	return 地区映射;
}

async function 批量补全优选地址地区前缀(优选地址列表 = [], env = {}) {
	const 条目列表 = (优选地址列表 || []).map(解析优选地址条目);
	const 待查询条目列表 = 条目列表
		.filter(item => item?.host && 备注需要补地区(item.remark, item.host))
		.map(item => ({
			host: item.host.replace(/^\[|\]$/g, '').trim(),
			port: item.port || '',
			remark: item.remark || '',
			key: `${item.host.replace(/^\[|\]$/g, '').trim()}:${item.port || ''}`
		}))
		.filter(item => item.host);
	log(`[地区前缀] 待补前缀条目=${待查询条目列表.length}，总优选条目=${优选地址列表.length}`);
	if (待查询条目列表.length === 0) return 优选地址列表;

	const 去重待查询条目列表 = Array.from(new Map(待查询条目列表.map(item => [item.key, item])).values());
	const 地区映射 = await 调用外部地区探测(去重待查询条目列表, env);
	log(`[地区前缀] 外部接口命中 ${地区映射.size}/${去重待查询条目列表.length}`);

	return (优选地址列表 || []).map((原始地址, index) => {
		const 条目 = 条目列表[index];
		if (!条目 || !备注需要补地区(条目.remark, 条目.host)) return 原始地址;
		const 纯净host = 条目.host.replace(/^\[|\]$/g, '').trim();
		const countryCode = 地区映射.get(`${纯净host}:${条目.port || ''}`) || 地区映射.get(纯净host);
		const 地址主体 = 条目.port ? `${条目.host}:${条目.port}` : 条目.host;
		return `${地址主体}#${countryCode || 'OFF'}-${条目.remark}`;
	});
}
